...
const getTargetFormatFromSlug = (slug: string): TargetFormat => {
    if (slug.endsWith('to-png')) return 'image/png'
    return 'image/jpeg' // Default to JPG
}

export function HeicConverter({ slug, title, description }: HeicConverterProps) {
    ...
    const convertFile = async (file: File): Promise<{ blob: Blob, name: string }> => {
        try {
            ...
            return { blob: finalBlob, name: `${fileNameWithoutExt}.${targetExtension}` }
        } catch (error) {
    ...
    return (
        <ToolShell title={title} description={description} icon={ImageIcon}>
            <div className="grid gap-6">
                <Card>
                    <CardHeader>
                        <CardTitle>Chọn file HEIC/HEIF</CardTitle>
                        <CardDescription>
                            Tải lên ảnh HEIC từ iPhone/iPad. Hỗ trợ chuyển đổi hàng loạt sang {targetExtension.toUpperCase()}.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="flex items-center gap-4">
                            ...
                            >
                                ...
                                <span className="text-muted-foreground text-sm font-medium">Nhấn để tải lên file HEIC/HEIF</span>
                                <input
                                    id="heic-upload"
                                    type="file"
                                    className="hidden"
